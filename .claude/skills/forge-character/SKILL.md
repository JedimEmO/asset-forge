...
| The GPU is free | `just gpu` | `holding   nobody` and `largest   trellis2 needs 22 GB (22528 MiB): fits`. Seen `does NOT fit — stop what holds the card before a generate` (exit 1): the `holding   pid N … GB  <process>` line names the holder. ACE-Step: `forge gen music --stop-server`. A studio window: close it. Anything else: by PID, never `pkill -f`. Never start a lift while another generate runs. |
