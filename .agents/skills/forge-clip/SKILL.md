...
| 5 verify | `audit`, `verify` | `verify`, `audit`, `manifest_check` | read-only gates, same behavior as CLI |
...
- **Idle prompting remains unreliable at the installed Core pin.** Guard,
  bounce, sway and shoulder-stretch prompts produced nearly static takes in
  the September 2026 trials. A paired raw/corrected control stayed static at
  text guidance 2 and 5 while jogging moved. Do not keep rerolling the failed
  guard example as a documented solution. Use a bounded trial; sparse authored
  constraints through `forge gen motion keys` are a separate route to evaluate.
...
| `STATIC` | `activity_m` < 0.035 **and** `avg_speed_mps` < 0.35 | low whole-body activity; inspect subtle idles on the real body before deciding |
...
- Text-only idle success is not established by the current trials. Keep the
  warning and inspect visible motion; authored keys are a separate input route.
