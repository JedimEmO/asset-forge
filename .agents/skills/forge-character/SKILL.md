...
| 7 verify | `verify`, `check-bodies` | `verify`, `audit`, `manifest_check` | read-only gates; audit includes body conformance |
...
For gear that biases bounding-box depth centering, inspect the mesh against the
skeleton and use `forge gen prepare ... --depth-offset=-0.12` only when measured.
The value is metres along glTF +Z (forward); negative moves the mesh backward.
It defaults to zero and is recorded as `depth_offset_m`. Skin's second prepare
replays recorded stature, yaw, depth offset and budget. Re-skin, export and
repeat every gate and strip after an adjustment; never shift the derived GLB.
The scrapyard scavenger is the measured -0.12 m example, not a default.
...
### Check a project-specific motion

`forge rig check <body.glb> --reference-clip <library-clip>` checks binding and
planted-foot clearance using that clip. Use this when the project has a run
or attack but no `walk`. An explicit missing name is refused; the default
contract reference still warns when absent. Keep the report with the posed
strip, because contact measurements do not judge deformation quality.
