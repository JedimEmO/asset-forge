...
#: The record kind. ``records.KINDS`` must carry it.
KIND = "ref"

# `records.py` is the Python half of the Rust record contract and belongs to
# whoever owns the schema; this door is the first writer of a `ref` record.
# Registering the kind here rather than editing that file keeps one owner per
# file, and the line disappears of its own accord the moment the kind is in
# the tuple upstream. It is a registration, not a widening: `normalize`
# still refuses every key the reader does not know.
if KIND not in records.KINDS:  # pragma: no cover - gone once records.py carries it
    records.KINDS = (*records.KINDS, KIND)
...
# `just ref-format rust` writes the Rust const the MCP tool's description
# reads, `just ref-format markdown` writes the block the forge-character
# skill includes — because three hand-maintained copies held to byte
...
def format_rust(const: str = "REFERENCE_FORMAT") -> str:
    """The same text as a Rust ``const``, for the MCP tool's description.

    Generated, never typed: ``just ref-format rust``.
    """
    body = format_text().rstrip("\n").replace("\\", "\\\\").replace('"', '\\"')
    # Each line ends `\n\`: the escaped newline is the text's own break and
    # the trailing backslash eats the source newline and the indent after it,
    # so the const reads the same in the file and in the terminal.
    rendered = [f"     {line}\\n\\" for line in body.splitlines()]
    rendered[-1] = f"     {body.splitlines()[-1]}"
    first = rendered.pop(0).lstrip()
    return (
        "// GENERATED by `just ref-format rust` from python/forge_gen/reference.py's FORMAT.\n"
        "// The format text has one home and this is a copy of it. Do not edit; regenerate.\n"
        "\n"
        "/// The reference format, verbatim from the door that enforces it.\n"
        f"pub const {const}: &str = \"{first}\n"
        + "\n".join(rendered)
        + "\";\n"
    )
...
RENDERERS = {"text": format_text, "rust": format_rust, "markdown": format_markdown}
...
def add_parser(subparsers) -> None:
    ...
    parser.add_argument("--print-format", choices=sorted(RENDERERS), metavar="text|rust|markdown", help="print the reference format and exit — the one home of that text")
...
def run(args) -> dict:
    ...
    if args.print_format:
        return {"_text": RENDERERS[args.print_format]()}
    settled = settle(args)
