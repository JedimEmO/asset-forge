"""Put SkinTokens' weights back on the profile's own armature, by joint order.
    ...
    python3 python/forge_gen/blender/spike_reattach.py \
            out/spike/<name>.skinned.glb --handed-in out/prepare/<name>.glb \
            [--out out/spike/<name>_skintokens.blend] [--profile DIR] [--json]

**A Phase 0 spike, not a door.** ``spike_skin.py`` measured what comes back
from ``demo.py --use_skeleton``: the weights are for our skeleton, but the
*names* are gone (``bone_0 … bone_54``) and every joint has moved — 7.6 mm
on average against a contract tolerance of 0.1 mm, because the checkpoint
tokenises joint positions on a 256-level grid. A body with those bones binds
nothing: ``forge rig check`` reports 0 driven, 27 orphaned, and the character
holds its rest pose through every clip in the library.

The two glbs agree on joint *order* — same count, identical parent array —
which is what ``spike_skin`` calls ``by_order`` alignment. So the workaround
this script measures is: take the per-vertex joint indices exactly as they
came back, read the name for index *i* off the skin that went in, and bind
the mesh to the profile's **untouched** armature out of ``rig.blend``. The
returned skeleton is discarded whole; nothing about it is repaired, averaged
or nudged. What ships out is the profile's rest pose, which is the pose every
clip in the library was baked against.

It goes out as a ``.blend`` on purpose, so the result is judged by the real
gate — ``forge gen export`` measures the rest pose against ``rig.blend``
joint for joint, and ``forge rig check`` then plays the walk on it. A spike
that wrote its own glb would be a second exporter with its own opinions.
"""
...
def run(args) -> dict:
    ...
    if len(returned) != len(original):
        raise InputRejected(
            ...
            "the order alignment spike_skin found does not hold, and there is no honest map by index"
        )
...
def run_in_blender(argv: list[str]) -> dict:
    ...
    if armature is None or armature.type != "ARMATURE":
        raise BackendFailed(f"{profile.rig_blend} holds no {armature_node!r} armature — the profile is broken")
    ...
    if len(meshes) != 1:
        raise InputRejected(f"{Path(args.skinned).name} imported {len(meshes)} mesh object(s) — the spike expects exactly one")
    body = meshes[0]
    ...
    # The returned armature is discarded whole: it is the thing the spike
    # found unusable, and keeping any of it would put a quantised rest pose
    # into a file the export gate measures against rig.blend.
    ...
    measured = {
        ...
        "rest_pose_from": str(profile.rig_blend),
...
def _main_outer(argv: list[str]) -> int:
    ...
    parser = argparse.ArgumentParser(prog="spike_reattach", description=__doc__, formatter_class=argparse.RawDescriptionHelpFormatter)
    _add_arguments(parser)
