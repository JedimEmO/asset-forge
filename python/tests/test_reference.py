...
def test_the_format_text_has_one_home_and_the_copies_are_generated():
    ...
    rust = reference.format_rust()
    assert rust.startswith("// GENERATED by `just ref-format rust`")
    assert "pub const REFERENCE_FORMAT: &str" in rust
    for line in text.splitlines():
        if line.strip():
            assert line.replace('"', '\\"') in rust
...
def test_print_format_writes_the_text_and_touches_nothing(project: Path):
    result = reference.run(_Args(print_format="text"))
    assert "A reference is one PNG" in result["_text"]
