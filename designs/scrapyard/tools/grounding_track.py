...
if __name__=="__main__":
 out=Path("designs/scrapyard/batch-02/grounding.json")
 pairs=[("scrapyard_scavenger","scrapyard_rifle_"+n) for n in ("aim","run","fire")]+[("scrapyard_rusher","scrapyard_rusher_"+n) for n in ("rush","attack")]
 result=[measure("assets/bodies/"+b+".glb","assets/clips/"+c+".glb") for b,c in pairs]
 out.write_text(json.dumps({"schema":"scrapyard-grounding-1","method":"Linear blend skinning, foot-dominated vertices, baked keys plus 60 Hz samples","application":"Interpolate offset at animation time and add to visual root Y. Blend this scalar with the same clip weights. Gameplay/controller position stays unchanged. Terrain and support-hand IK are separate.","clips":result},indent=2)+"\n")
 for row in result:print(row["animation"],"maximum visual-root lift",row["max_lift_m"])
