...
Use the project-specific `.forge/mcp.json` generated by `forge init` or
`forge agent-config`. Its absolute executable and explicit project argument keep
the client's working directory from selecting another library.

The server exposes 30 tools, including read-only `verify`, `audit` and
`manifest_check`, with the same gates as CLI. Validation replies include
`passed`, `exit_code`, `code` and `report`; failed gates are MCP error results.

The workflow guide is listed through `resources/list` and readable at
`forge://guides/v1/workflow`, including before project initialization. It is
embedded in the binary. Initializing the server's bound directory activates its
project and queue on this connection, using the chosen hardware tier. Creating
another directory does not switch the session.
